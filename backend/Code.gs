...
function validateUser(name, dob) {
  try {
    ...
    if (!usersSheet) {
      return { success: false, message: 'Users sheet not found. Please contact administrator.' };
    }
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      const hasPicked = row[3] ? row[3].toString().toLowerCase() === 'true' : false;
...
function pickRecipient(userId, name, dob) {
  try {
    ...
    if (!usersSheet || !assignmentsSheet) {
      return { success: false, message: 'Required sheets not found. Please contact administrator.' };
    }
    ...
    if (userData[userRow][3] && userData[userRow][3].toString().toLowerCase() === 'true') {
      return {
        success: false,
        message: 'You have already picked your Secret Santa!'
      };
    ...
    for (let i = 1; i < assignmentData.length; i++) {
      if (assignmentData[i][2]) { // RecipientID column
...
function setupSampleData() {
  ...
  if (!usersSheet) {
    ...
    usersSheet.appendRow(['Name', 'DOB', 'UserID', 'HasPicked', 'PickedRecipient']);
  ...
  if (!assignmentsSheet) {
    ...
    assignmentsSheet.appendRow(['GiverID', 'GiverName', 'RecipientID', 'RecipientName', 'Timestamp']);
